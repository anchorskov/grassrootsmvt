...
var ALLOWED_ORIGINS = [
  ...
  "http://127.0.0.1:8788",
  // local direct
  /^https:\/\/[a-z0-9-]+\.grassrootsmvt\.pages\.dev$/
  // preview deployments
];
var DEBUG_CORS = globalThis.DEBUG_CORS === "true";
function getCorsHeaders(envOrRequest, maybeOrigin) {
  let origin;
  let env;
  if (maybeOrigin !== void 0) {
    env = envOrRequest;
    origin = maybeOrigin;
  } else {
  ...
  const envAllowed = envAllowedRaw ? envAllowedRaw.split(",").map((o) => o.trim()) : [];
  const allAllowed = [...ALLOWED_ORIGINS, ...envAllowed];
  const isAllowed = allAllowed.some(
    (allowed) => typeof allowed === "string" ? allowed === origin : allowed instanceof RegExp && allowed.test(origin)
  );
  if (origin && isAllowed) {
    return {
  ...
  if (DEBUG_CORS) {
    console.warn("[CORS] \u{1F6AB} Blocked origin:", origin);
  }
  return null;
}
...
var onRequestOptions = /* @__PURE__ */ __name(({ request }) => handleCorsPreflight(request), "onRequestOptions");
...
// _utils/verifyAccessJWT.js
async function verifyAccessJWT(request, env) {
  const token = getAccessJWT(request);
  if (!token) {
    return { valid: false, error: "Missing Access token" };
  }
  const teamDomain = env.TEAM_DOMAIN || (env.CF_ACCESS_TEAM ? `https://${env.CF_ACCESS_TEAM}.cloudflareaccess.com` : null);
...
__name(getAccessJWT, "getAccessJWT");
function getCookie(request, name) {
  const cookie = request.headers.get("cookie");
  if (!cookie) return null;
  const match2 = cookie.match(new RegExp(`(?:^|;\\s*)${name}=([^;]+)`));
...
// api/healthz.js
var onRequestGet5 = /* @__PURE__ */ __name(() => new Response(JSON.stringify({ ok: true, from: "pages-functions" }), {
  headers: { "content-type": "application/json" }
}), "onRequestGet");
...
var onRequestOptions4 = /* @__PURE__ */ __name(() => new Response(null, { status: 204 }), "onRequestOptions");
...
// _middleware.js
function onRequestOptions6({ request }) {
  let cors = {};
  try {
...
// ../../.wrangler/tmp/pages-5rLbod/functionsRoutes-0.860570580464677.mjs
var routes = [
  {
...
// ../../../../.npm/_npx/b31d77f56277ab93/node_modules/path-to-regexp/dist.es2015/index.js
function lexer(str) {
  var tokens = [];
  var i = 0;
  while (i < str.length) {
...
// ../../../../.npm/_npx/b31d77f56277ab93/node_modules/wrangler/templates/pages-template-worker.ts
var escapeRegex = /[.+?^${}()|[\]\\]/g;
